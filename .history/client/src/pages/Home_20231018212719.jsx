...
const Home = () => {
  ...
  const completedTodo = useCallback(
    async (id) => { // Accept the 'id' as a parameter
      try {
        await axios
          .put(
            `http://localhost:4000/api/todo/completed/${id}`,
            null, // Pass the 'id' in the URL, so the request body can be empty
            {
              headers: { "Content-Type": "application/json" },
            }
          )
          .then((response) => {
            // Update the 'todos' state correctly using the callback function
            ...
            {todos.map((todo, index) => {
              return (
                <div key={index} className="my-4">
                  <div
                    className={`rounded-lg flex flex-col lg:flex-row justify-between
                 hover:bg-blue-200 px-5 border-l-[1px] border-blue-600 items-baseline `}
                  >
